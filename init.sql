...
CREATE TABLE mechanisms (
    id SERIAL PRIMARY KEY,
    family_id INT REFERENCES families(id) ON DELETE CASCADE, -- ON DELETE CASCADE
    name VARCHAR(255) UNIQUE NOT NULL, -- UNIQUE constraint
    description TEXT,
    created_by VARCHAR(255),
    created_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP
...
CREATE TABLE mechanism_versions (
    id SERIAL PRIMARY KEY,
    mechanism_id INT REFERENCES mechanisms(id) ON DELETE CASCADE, -- ON DELETE CASCADE
    version_number INT NOT NULL,
    tag VARCHAR(50),
    created_by VARCHAR(255),
    published_date TIMESTAMP,
    UNIQUE (mechanism_id, version_number) -- Ensures unique version numbers per mechanism
);

-- Table to store relationships between reactions and species
CREATE TABLE reaction_species (
    id SERIAL PRIMARY KEY,
    reaction_id INT REFERENCES reactions(id) ON DELETE CASCADE, -- ON DELETE CASCADE
    species_id INT REFERENCES species(id) ON DELETE CASCADE, -- ON DELETE CASCADE
    role VARCHAR(50) NOT NULL CHECK (role IN ('reactant', 'product')), -- Enforces valid roles
    UNIQUE (reaction_id, species_id, role) -- Prevents duplicate role assignments
);
...
CREATE TABLE mechanism_reactions (
    ...
    mechanism_id INT REFERENCES mechanisms(id) ON DELETE CASCADE, -- ON DELETE CASCADE
    reaction_id INT REFERENCES reactions(id) ON DELETE CASCADE, -- ON DELETE CASCADE
    UNIQUE (mechanism_id, reaction_id) -- Prevents duplicate mappings
);
...
CREATE TABLE mechanism_species (
    ...
    mechanism_id INT REFERENCES mechanisms(id) ON DELETE CASCADE, -- ON DELETE CASCADE
    species_id INT REFERENCES species(id) ON DELETE CASCADE, -- ON DELETE CASCADE
    UNIQUE (mechanism_id, species_id) -- Prevents duplicate mappings
);

-- New table to store initial conditions specific to species within a mechanism
CREATE TABLE initial_conditions_species (
    ...
    UNIQUE (mechanism_id, species_id) -- Ensures one initial condition per species per mechanism
);
...
CREATE TABLE user_mechanisms (
    id SERIAL PRIMARY KEY,
    user_id INT REFERENCES users(id) ON DELETE CASCADE, -- ON DELETE CASCADE
    mechanism_id INT REFERENCES mechanisms(id) ON DELETE CASCADE, -- ON DELETE CASCADE
    role VARCHAR(50), -- Role could be 'owner', 'contributor', etc.
    UNIQUE (user_id, mechanism_id, role) -- Prevents duplicate role assignments
);
...
-- Insert species, ensuring no duplicates by using ON CONFLICT DO NOTHING
INSERT INTO species (name, description, created_by) VALUES
...
INSERT INTO reactions (equation, description, created_by) VALUES
